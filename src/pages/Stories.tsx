...
export function Stories({ onNavigate }: StoriesProps) {
  // Mock user stories - filter by current user
      ...
      <Tabs defaultValue="published" className="w-full">
        <TabsList className="mb-6">
          ...
          <TabsTrigger value="unlisted">
            Unlisted ({unlistedStories.length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="drafts">
          {draftStories.length > 0 ? (
            ...
            <EmptyState
              title="No drafts"
              description="Your draft stories will appear here"
            />
          )}
        </TabsContent>
        ...
        <TabsContent value="unlisted">
          {unlistedStories.length > 0 ? (
            ...
            <EmptyState
              title="No unlisted stories"
              description="Stories set to unlisted will appear here"
            />
          )}
        </TabsContent>
      </Tabs>
...
function StoryItem({ story, onNavigate }: { story: Post; onNavigate: (page: string, postId?: string) => void }) {
    ...
    <div className="flex gap-4 p-4 border rounded-lg hover:bg-gray-50 transition-colors">
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-4 mb-2">
          <h3 className="line-clamp-2 cursor-pointer hover:underline" onClick={() => onNavigate('post-detail', story.id)}>
            {story.title}
          </h3>
          {story.status === 'draft' && (
      ...
      <div className="flex items-start gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => onNavigate('post-detail', story.id)}>
